        ...
        public static void Log(string message, LogLevel ll, Nullable<NotificationType> nt = null, string email = null)
        {
            ...
            catch(Exception ex)
            {
                ...
                catch(Exception exc)
                {
                    ...
                    {
                        if (!System.IO.Directory.Exists("c:\\inetpub\\vhosts\\grubclub.com\\httpdocs\\jobs\\WishListFollow\\Log\\Error"))
                            System.IO.Directory.CreateDirectory("c:\\inetpub\\vhosts\\grubclub.com\\httpdocs\\jobs\\WishListFollow\\Log\\Error");
                        string path = "c:\\inetpub\\vhosts\\grubclub.com\\httpdocs\\jobs\\WishListFollow\\Log\\Error" + DateTime.Now.ToShortDateString().Replace("/", "-") + ".txt";
                        if (!File.Exists(path))
                        {
                            // Create a file to write to. 
                            File.WriteAllText(path, DateTime.Now.ToString() + "\t" + ex1.Message + "\t" + ex1.StackTrace + "\t" + ex1.InnerException + "\n");
                        }
                        else
                            // This text is always added
                            File.AppendAllText(path, DateTime.Now.ToString() + "\t" + ex1.Message + "\t" + ex1.StackTrace + "\t" + ex1.InnerException + "\n");
                    }
                }
            }
        }
