-- Author:	Swati
-- Summary:	Updated report to include 2015 data

DECLARE @ScriptCode nvarchar(50)
SET @ScriptCode='2016.05.06.UpdateRevenueTargetReport'
...
update report
set name='Revenue Target Report', query='
DECLARE @YearStartDate DATETIME, @StartCurrentMonth DATETIME
SET @YearStartDate = ''01-jan-2015''--DATEADD(yy, DATEDIFF(yy,0,getdate()), 0) 
...
from(
select [year], Datename(month, Dateadd(month, [MONTH], 0) - 1) AS [month],EventCount, ChefCount, guests,reservedseats, eventCost, numberofguests, cost, guestbaseprice, totalPrice, case when numberofguests = 0 then 0 else guestbaseprice/numberofguests end AS AverageTicketPrice
 from
