-- Author:	Swati
-- Summary:	Added logic to filter price chnage before event approval

DECLARE @ScriptCode nvarchar(100)
SET @ScriptCode='2016.05.03.UpdateEventCommissionChangeLogReport'
...
FROM   ((SELECT l.eventid, 
        ...
        (SELECT l.eventid, 
         ...
         FROM   [dbo].[eventpricechangelog] l with(nolock)
				...
				LEFT JOIN (select l.eventid, min(ea.bookingdate) AS BookingDate FROM [dbo].[eventpricechangelog] l with(nolock) join [dbo].[EventAttendee] ea with(nolock)
					  ON l.eventid = ea.eventId group by l.eventid) ea on l.eventid = ea.eventId
				WHERE l.date > isnull(e.dateapproved, isnull(ea.bookingdate,''01-jan-1900''))))a 
ORDER  BY eventid, 
          date '
