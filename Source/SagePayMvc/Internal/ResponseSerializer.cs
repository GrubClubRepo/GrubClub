	...
	public class ResponseSerializer {
		...
		public void Deserialize(Type type, string input, object objectToDeserializeInto) {
			...
			foreach (var nameValuePairCombined in bits) {
                ...
                else if (prop.PropertyType == typeof(ThreeDSecureStatus)){
                    convertedValue = ConvertStringToSagePayThreeDSecureStatus(value);
                }
				else {
		...
		public static ResponseType ConvertStringToSagePayResponseType(string input) {
			if (!string.IsNullOrEmpty(input)) {
                ...
                if (input.StartsWith("3DAUTH"))
                {
                    return ResponseType.ThreeDAuth;
                }
			}
			return ResponseType.Unknown;
		}

        public static ThreeDSecureStatus ConvertStringToSagePayThreeDSecureStatus(string input)
        {
            if (!string.IsNullOrEmpty(input))
            {
                if (input.StartsWith("OK"))
                {
                    return ThreeDSecureStatus.Ok;
                }

                if (input.StartsWith("NOAUTH"))
                {
                    return ThreeDSecureStatus.NoAuth;
                }

                if (input.StartsWith("CANTAUTH"))
                {
                    return ThreeDSecureStatus.CantAuth;
                }

                if (input.StartsWith("NOTAUTHED"))
                {
                    return ThreeDSecureStatus.NotAuthed;
                }

                if (input.StartsWith("ATTEMPTONLY"))
                {
                    return ThreeDSecureStatus.AttemptOnly;
                }

                if (input.StartsWith("NOTCHECKED"))
                {
                    return ThreeDSecureStatus.NotChecked;
                }

                if (input.StartsWith("INVALID"))
                {
                    return ThreeDSecureStatus.Invalid;
                }

                if (input.StartsWith("INCOMPLETE"))
                {
                    return ThreeDSecureStatus.InComplete;
                }

                if (input.StartsWith("MALFORMED"))
                {
                    return ThreeDSecureStatus.Malformed;
                }
                if (input.StartsWith("INVALID"))
                {
                    return ThreeDSecureStatus.Invalid;
